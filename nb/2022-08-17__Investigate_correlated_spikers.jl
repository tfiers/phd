...
ax = ydistplot(
    ...
    jn("Unconnected", "but detected", "↕", "exc inputs") => spikecors(signif_unconn, ii.exc_inputs),
    jn("Unconnected", "but detected", "↕", "inh inputs") => spikecors(signif_unconn, ii.inh_inputs),
    figsize = (6, 3),
...
ax.get_children() .|> (ch -> ch.set_clip_on(true))
ax.figure
