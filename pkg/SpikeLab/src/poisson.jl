...
Exponential distribution with rate parameter ``r``, accumulating them until ``T`` is
reached. The number of spikes ``N`` in ``[0, T]`` will be Poisson-distributed, with mean =
``rT``.
