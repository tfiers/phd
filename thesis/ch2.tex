...
\marginpar{
    \vspace*{1em}
    \includegraphics[w=1]{impulse_response_inh}
    \captionof{figure}{
        \textbf{Inhibitory impulse response of the conductance-based AdEx neuron},
        % \small{using a single excitatory input spike arriving at $t = 10$~ms, with a synaptic weight $Δg_\exc$ of 14 pS. Parameters as in \cref{tab:AdEx-params}, with $E_\exc = 0$~mV and $τ_g = 7$~ms. More details at \nburl{2023-07-26__AdEx_Nto1_we_I_syn}.}
        }
    \label{fig:impulse_response_inh}
}
...
\Cref{fig:impulse_response_inh} does the same thing, but for an inhibitory spike, and with a synaptic weight ($Δg_\inh$) four times as large. Note that, even though the change in the synaptic conductance is so much larger, the size of the synaptic current $I_\syn$, and the PSP bump (in the voltage $V$), are not larger. (In fact, they are a smidge smaller). This is due to the fact that \emph{(put explanation at end of 'sec:synaptic_weights' here, and there, ref to here)}.
