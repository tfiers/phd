...
We solve the differential AdEx equations using first-order (Euler) integration, with a timestep $Δt$ of $0.1$ ms. See \cref{sec:software} below for more details on the numeric implementation.
...
Combining \cref{eq:AdEx-V,eq:AdEx-w,eq:AdEx-reset} with \cref{eq:I_syn_simple,eq:dge-dgi}, our complete conductance-based, AdEx point neuron model is:
...
\marginpar{
    % \vspace*{-1em}
    \includegraphics[w=1]{impulse_response}
    \captionof{figure}{\small{Response of the conductance-based AdEx neuron to a single excitatory input spike arriving at $t = 10$~ms, with a synaptic weight $Δg_\exc$ of 14 pS. Parameters as in \cref{tab:AdEx-params}, with $E_\exc = 0$~mV and $τ_g = 7$~ms. More details at \nburl{2023-07-26__AdEx_Nto1_we_I_syn}.}}
    \label{fig:impulse_response}
}
\begin{alignat*}{2}
    & \text{When}\ V > θ \text{:}  &&   V ← V_r \\
    & && w ← w + b \\[1em]
    & \text{On spike at an exc. synapse:} \quad && \ g_\exc ← g_\exc + Δg_\exc \\
    & \text{On spike at an inh. synapse:}  \quad && \ g_\inh ← g_\inh + Δg_\inh
\end{alignat*}

\Cref{fig:impulse_response} shows the impulse response of this model, for a single incoming excitatory spike. The PSP bump is visible in the third panel. Note its tiny size, of about 0.04~mV. Our task will be to detect this tiny signal, in a sea of other such PSP bumps and voltage imaging noise.
...
Given these data, we choose our parameters $μ$ and $σ$ so that the distribution lies roughly halfway O'Connor's and Roxin's, while making sure our firing rates are rather low than high: as our idea for connection inference rests on the number of spikes that can be used to calculate spike-triggered averages, we don't want to overestimate the number of available input spikes and obtain overly optimistic results.
