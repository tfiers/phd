...
\begin{table}[h]
    \vspace*{1.8em} % making space for footnote above
    \begin{sidecaption}
...
\Cref{fig:exIF-fit} shows the $F(V)$ curve for an AdEx neuron fit to a real neuron. \Cref{fig:dynsys} compares the $F(V)$ curve of an AdEx neuron  with that of another two-dimensional neuron model, the Izhikevich neuron.
...
We see in \cref{eq:AdEx-V} (for $I_\syn = 0$ and $w = 0$) that the leak potential $E_L$ is, numerically speaking (though not strictly mathematically), indeed a fixed point: at realistic parameter values, the exponential term is negligibly small at $V = E_L$.\footnote{
...
\newpage  % for footnote clippin otherwise
...
After a comparison of compilation and runtimes of the Brian and Julia implementations (\cref{tab:software-timings}), we decided to use the software written in Julia.\footnoteatfoot{
    The Julia implementation is considerably faster than the implementation in `standalone' Brian, which  uses C++ and should thus be comparable in speed. This might be due to me not using Brian optimally, or to the fact that Brian is a general purpose spiking neural network simulator with much more capabilities; and our software was written specifically for the N-to-1 AdEx problem.
}
