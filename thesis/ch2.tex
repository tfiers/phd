...
I.e, unlike what is suggested by the `$T$' subscript, and the name `effective threshold potential' given to it in \cite{Naud2008FiringPatternsAdaptive}, $V_T$ is not the instantaneous threshold potential (it is not a zero of $F$), but rather the minimum of $F$ (it is the zero of $\d[V]{F}$).

The minimum of Izhikevich's $F$ is easily found as the average of the parabola's two zeros.
We thus find our second nontrivial correspondence between an Izhikevich and an AdEx neuron:
\begin{equation}
    V_T = (E_L + E_T) / 2
\end{equation}

Where then is the actual instantaneous firing threshold for the AdEx neuron?
There is no solution using only elementary functions. But using the Lambert $W$ or `product logarithm' functions (see \cref{fig:LambertW}), the roots of AdEx's $F$ are at:
\begin{equation}
    V = E_L - Δ_T W_k \left( -\exp\left( \frac{E_L - V_T}{Δ_T} \right) \right) \label{eq:LambertW}
\end{equation}
..where $k = 0$ gives the resting potential, and $k = -1$ the instantaneous firing threshold.\\
The argument of the exponential is negative, and hence the argument of the $W$ function is very small. $W_0$ passes through zero; hence the actual resting potential -- though not exactly equal to it -- is very close to $E_L$.
...
Given that Izhikevich's $F$ is a parabola, with equal slopes at both roots, we already know the slope of its $F$ at the firing threshold: it is the same as the leak conductance, \cref{eq:leak_conductance}. For AdEx's $F$, \cref{eq:dFdV} gives us its slope at the firing threshold $E_T$ (i.e. at \cref{eq:LambertW} for $k = -1$):
\begin{equation}
    g_L \left( \exp\left(\frac{E_T - V_T}{Δ_T}\right) - 1 \right)
\end{equation}
