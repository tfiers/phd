...
\begin{figure}
    \begin{subfloat}
        \includegraphics{residuals_vs_time.png}
        \includegraphics{residuals_distplot.png}
    \end{subfloat}
    \captionn
        {Residuals of an example regression}
        {
        Linear regression of the first 10 ms of the spike-triggered average of an AdEx neuron with 6500 inputs simulated for 10 minutees, for its highest-firing excitatory input.
        Left: Residuals over time.
        Right: Distribution of residuals marginalized over all timepoints. Vertical black lie indicates both the mean ($-5.9E-12$~mV) and the median ($0.1$~mV).\\
        The residuals steem homoskedastic and unbiased, but are not symmetrically distributed.\\
